...
export default class PostsController {
  ...
  @Get("/about")
  async getRecommendCategory(@Query("search") search: string, @Query("pageSize") pageSize: integer, @Query("currentPage") currentPage: integer,
                             @Query("brand") brand: string, @Query("eventtype") eventtype: string, @Query("category") category: string,
                             @Query("sort") sort: string, @Res() res: Response) {
    if (search) {
      const searchResult = await this.postsService.categorySearch(search, pageSize, currentPage, brand, eventtype, category, sort);
      if (searchResult !== "") {
        return res.status(200).json(searchResult);
      } else {
        return res.status(401).json({ message: "앗! 아쉽게도 검색된 상품이 없습니다." });
      }
    } else {
      return res.status(401).json({ message: "앗! 아쉽게도 검색된 상품이 없습니다." });
    }
